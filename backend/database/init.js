...
async function initializeDatabase() {
	try {
		const sqlSchemaPath = path.join(__dirname, "schema/schema.sql");

		const sqlSchema = fs.readFileSync(sqlSchemaPath, "utf8");

		const sqlIndexesPath = path.join(__dirname, "indexes/indexes.sql");

		const sqlIndexes = fs.readFileSync(sqlIndexesPath, "utf-8");
