...
describe("GeoSpatialService", () => {
    ...
    test("_haversineDistanceKm computes approximate distance correctly", () => {
        const distance = GeoSpatialService._haversineDistanceKm(-33.86, 151.21, -33.87, 151.22);
    ...
    test("findAlertsInBoundingBox returns rows for marker and polygon ids", async () => {
        ...
        AlertPolygonDAO.all.mockResolvedValue([{ alert_id: 1 }, { alert_id: 2 }]);
        AlertDAO.get.mockResolvedValue({ count: 2 });
        AlertDAO.all.mockResolvedValue([{ id: 1 }, { id: 2 }]);

        const result = await GeoSpatialService.findAlertsInBoundingBox({ minLat: -34, maxLat: -33, minLng: 151, maxLng: 152 }, {}, { limit: 10, offset: 0 });
    ...
    test("findAlertsNearby filters rows by radius using distance calculation", async () => {
        ...
        const result = await GeoSpatialService.findAlertsNearby(-33.86, 151.21, 2, {}, { limit: 10, offset: 0 });
