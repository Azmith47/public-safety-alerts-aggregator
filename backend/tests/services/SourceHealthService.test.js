...
const LookupService = { getOrCreateSource: jest.fn() };
const SourceHealthDAO = {
	getBySourceId: jest.fn(),
	create: jest.fn(),
	updateBySourceId: jest.fn(),
	getAll: jest.fn(),
};
jest.unstable_mockModule("../../services/LookupService.js", () => ({
	default: LookupService,
}));
jest.unstable_mockModule("../../database/dao/SourceHealthDAO.js", () => ({
	default: SourceHealthDAO,
}));

const { default: SourceHealthService } =
	await import("../../services/SourceHealthService.js");
...
describe("SourceHealthService", () => {
	...
	test("_normalizeMessage converts objects to JSON strings and truncates long texts", () => {
		...
		const normalized = SourceHealthService._normalizeMessage({
			message: "hello",
		});
		expect(normalized).toContain("hello");
		expect(SourceHealthService._normalizeMessage(longText).length).toBe(
			1000,
		);
	});
	...
	test("recordRun creates a new health record when none exists", async () => {
		...
		const result = await SourceHealthService.recordRun(
			"RFS",
			"https://example.com",
			{ success: true, processed: 2, created: 1 },
		);
	...
	test("recordRun updates an existing health record on failure", async () => {
		...
		SourceHealthDAO.getBySourceId.mockResolvedValue({
			run_count: 1,
			success_count: 1,
			failure_count: 0,
			processed_count: 2,
		...
		const result = await SourceHealthService.recordRun("TFNSW", null, {
			success: false,
			processed: 3,
			failed: 1,
		});
		...
		expect(SourceHealthDAO.updateBySourceId).toHaveBeenCalledWith(
			8,
			expect.objectContaining({
				last_status: "failure",
				run_count: 2,
