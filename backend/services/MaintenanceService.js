...
class MaintenanceService {
	constructor() {
		this.alertRetentionDays =
			parseInt(process.env.ALERT_RETENTION_DAYS, 10) || 30;
		this.notificationRetentionDays =
			parseInt(process.env.NOTIFICATION_RETENTION_DAYS, 10) || 30;
	}
	...
	async _deleteAlertData(alertIds) {
		...
		await AlertMarkerDAO.delete(
			AlertMarkerDAO.tableName,
			`alert_id IN (${placeholders})`,
			alertIds,
		);
		await AlertPolygonDAO.delete(
		...
		await AlertRegionDAO.delete(
			AlertRegionDAO.tableName,
			`alert_id IN (${placeholders})`,
			alertIds,
		);
	...
	async cleanupExpiredAlerts(days = this.alertRetentionDays) {
		...
		if (!expiredAlertIds.length) {
			return {
				expiredAlerts: 0,
				message: `No alerts older than ${days} days were found.`,
			};
		}
