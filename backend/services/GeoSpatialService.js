...
class GeoSpatialService {
    ...
    async _queryAlertsByIds(alertIds, filters = {}, options = {}) {
        ...
        const rows = await AlertDAO.all(`SELECT * FROM alerts ${whereClause} ${orderClause} LIMIT ? OFFSET ?`, params.concat([limit, offset]));
        return { total, rows };
    }
    ...
    async findAlertsNearby(lat, lng, radiusKm, filters = {}, options = {}) {
        ...
        const kmPerDegreeLng = Math.abs(111.32 * Math.cos(latRadians)) || 111.32;
        ...
        const markerIds = markerRows
            .filter(row => this._haversineDistanceKm(lat, lng, row.latitude, row.longitude) <= radiusKm)
    ...
    async getAlertGeometry(alertId) {
        ...
        const markers = await AlertMarkerDAO.findAll(AlertMarkerDAO.tableName, 'alert_id = ?', [alertId]);
        const polygons = await AlertPolygonDAO.findAll(AlertPolygonDAO.tableName, 'alert_id = ?', [alertId]);
...
class GeoSpatialService {
	...
	async _collectMatchIdsFromMarkerBox(box) {
		...
		try {
			const rows = await AlertMarkerDAO.all(sql, params);
			return rows.map((row) => row.alert_id);
		} catch (error) {
			console.error("MARKER: DAO error:", error);
			throw error;
		}
	}
	...
	async _collectMatchIdsFromPolygonBox(box) {
		const sql = `
			SELECT DISTINCT alert_id
			FROM ${AlertPolygonDAO.tableName}
			WHERE latitude BETWEEN ? AND ?
			AND longitude BETWEEN ? AND ?`;
		...
		try {
			const rows = await AlertPolygonDAO.all(sql, params);
			return rows.map((row) => row.alert_id);
		} catch (error) {
			console.error("POLYGON: DAO error:", error);
			throw error;
		}
	}
	...
	async _collectMatchIdsFromPolylineBox(box) {
		const sql = `
			SELECT DISTINCT alert_id
			FROM ${AlertPolylineDAO.tableName}
			WHERE latitude BETWEEN ? AND ?
			AND longitude BETWEEN ? AND ?`;
		...
		try {
			const rows = await AlertPolylineDAO.all(sql, params);
			return rows.map((row) => row.alert_id);
		} catch (error) {
			console.error("POLYLINE: DAO error:", error);
			throw error;
		}
	}
