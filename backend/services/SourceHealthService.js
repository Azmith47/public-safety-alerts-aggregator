...
class SourceHealthService {
    ...
    _normalizeMessage(message) {
        ...
        return text.length > this.maxMessageLength ? text.slice(0, this.maxMessageLength) : text;
    }
    ...
    async recordRun(sourceName, sourceWebsite, result = {}) {
        ...
        const message = this._normalizeMessage(result.error || result.message || null);
        ...
        const updateData = {
            ...
            processed_count: (existing?.processed_count || 0) + (result.processed || 0),
            created_count: (existing?.created_count || 0) + (result.created || 0),
            updated_count: (existing?.updated_count || 0) + (result.updated || 0),
